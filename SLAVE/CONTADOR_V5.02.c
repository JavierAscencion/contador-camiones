...
int size_s;//Tamaño de trama que espera leer el puerto serial (se utiliza para agilizar la captura de datos)
...
//!
   ...
   if( (bloq_p1)&&(tim_p1>=time_bloqueo)&&(!en_b1) ) {//bloqueo de par 1 por más de X segundos
   ...
   if( (bloq_p2)&&(tim_p2>=time_bloqueo)&&(!en_b2) ) {//bloqueo de par 2 por más de X segundos
       bloq_p2=0;
   ...
   if( (bloq_p3)&&(tim_p3>=time_bloqueo)&&(!en_b3) ) {//bloqueo de par 3 por más de X segundos
       bloq_p3=0;
       if( (!en_b1)&&(!en_b2)&&(!en_b3) ){
            lcd_gotoxy(10,1);//cronometro para limpiar
            lcd_putc("BL");
...
int evaluar_string(char *command[]){
  ...
  // fprintf(monitor,">");
   ...
   //lcd_gotoxy(9,2);
   //printf(lcd_putc,":%d",conteo_command);//pinta en lcd si el numero de caso detectado (9 significa que no es un dato valido)
